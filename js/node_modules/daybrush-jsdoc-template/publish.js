...
function buildNavUl(file, header, type, members) {
    ...
    itemsNav += members.map(function (member) {
        ...
        const fragmendId = member.fragmendId;

        return `<li data-type='${type}'>${linkto(member.longname, removeExports(member.name), "", fragmendId)}</li>`;
    }).join("");
        ...
        items.forEach(function(item) {
            ...
            if ( !hasOwnProp.call(item, 'longname') ) {
                ...
                const options = defs.map(def => {
                    ...
                    if (tags.some(tag => tag.title === "options")) {
                        const properties = def.properties;
                        const optionsName = def.name;

                        return properties.map(property => {
                            const name = property.name;

                            return {
                                fragmendId: `.${optionsName}-${name}`,
                ...
                if (options.length) {
                    itemsNav += buildNavUl(item.longname, "Options", "options", options);
                }
    ...
    data().each(function(doclet) {
        ...
        if (doclet.properties) {
            doclet.properties.forEach(function (prop) {
                if (typeof prop.see === "string") {
                    prop.see = [hashToLink(doclet, prop.see)];
                }
            });
        }
        if (doclet.see) {
