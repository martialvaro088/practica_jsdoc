...
exports.registerImport = function(filename, extra) {
    var dir = filename.split('/');

    dir.pop();
    dir = dir.join('/');

    if (extra.source.value.indexOf('.') !== 0) {
        dir = extra.source.value;
    } else {
        dir = path.resolve(dir, extra.source.value);
    }

    var imports = typeMap.imports;

    imports[filename] = imports[filename] || {};

    extra.specifiers.forEach(function(specifier) {
        imports[filename][specifier.alias] = {
            key: specifier.key,
            source: dir
        };
    });
};
exports.registerExport = function(filename, type, longtype) {
    var exports = typeMap.exports;

    exports[filename] = exports[filename] || {};
    if (!exports[filename][type]) {
        exports[filename][type] = longtype.replace(/#|~/g, '.');
    }
};


(function() {
    var _linkMap = (env.conf && env.conf.linkMap) || {};
    var _name;

    // eslint-disable-next-line guard-for-in
    for (_name in _linkMap) {
        registerLink(_name, _linkMap[_name]);
    }
})();
...
function traceType(filename, longtype) {
    var imports = typeMap.imports;
    var exports = typeMap.exports;
    var currentImports = imports[filename];
    var types = longtype.split('.');
    var length = types.length;
    var type = types[length - 1];
    var rv = longtype;
    var alias = types[0];

    if (currentImports && currentImports[alias]) {
        var key = currentImports[alias].key;
        var source = currentImports[alias].source;

        if (key === '*') {
            key = type;
        }
        for (var j in exports) {
            if (j.indexOf(source) === 0 && exports[j][key]) {
                var exportKey = exports[j][key];

                if (key === 'default' && exports[j][exportKey]) {
                    rv = exports[j][exportKey];
                } else if (imports[j] && imports[j][exportKey]) {
                    rv = traceType(j, (imports[j][exportKey][key] === "*" ? types.slice(1) : types).join("."));
                    continue;
                } else {
                    rv = exportKey;
                }
    ...
    } else if (exports[filename] && exports[filename][type]) {
        rv = exports[filename][type];
    }
...
function buildLink(longname, linkText, options) {
    ...
    if (longname.indexOf('$ts:') === 0) {
        var result = /<file>(.*?)<\/file>/g.exec(longname);
        var filename = '';
        var buildLinkMap = options && options.linkMap;

        if (result) {
            longname = longname.replace(result[0], '');
            filename = result[1];
        }
        return htmlsafe(longname.replace('$ts:', '')).replace(/(("[^"]*")|(&*[a-zA-Z0-9.]{2,};*))/g, function(type) {
            if (type === '&amp;' || type === '&lt;' || type[0] === '"' || type[0] === "'" || type === '...') {
                return type;
            }
            var rest = type.indexOf("...") === 0 ? "..." : "";
            var longtype = traceType(filename, type.replace(/^\.\.\./g, ""));

            if (buildLinkMap && buildLinkMap[longtype]) {
                var target = '';

                if ((/^http(s)*:\/\//g).test(buildLinkMap[longtype])) {
                    target = ' target="_blank"';
                }

                return rest + '<a href="' + buildLinkMap[longtype] + '"' + target + '>' + type + '</a>';
            } else {
                return type;
            }
        });
